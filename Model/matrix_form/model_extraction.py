...
def main_comp():
    ...
    output = np.zeros((test_array.shape[0], 2))
    lstm_forward = lstm_matrix_inference(W_xh, W_hh, b_h, batch_size)
    fc_forward = fc_matrix_inference(W, b)
    model.eval().to('cuda')
    model_output = np.zeros((test_array.shape[0],2))
    for i in range(test_array.shape[0]):
        output[i] = fc_forward(lstm_forward(test_array[i]))
        model_output[i] = model(torch.tensor([test_array[i]]).to('cuda', torch.float32)).cpu().detach()
    for i in range(10):
